...
export function EmployeeImportModal({
  ...
  const handleUploadSubmit = async () => {
    ...
    try {
      ...
      if ((reviewData?.error_count ?? 0) > 0 || (Array.isArray(reviewData?.errors) && reviewData.errors.length > 0)) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 overflow-y-auto">
      ...
      {/* Modal Container — Spacious Widescreen Layout (max-w-3xl) */}
      <div className="bg-white rounded-3xl border border-gray-200/90 shadow-2xl max-w-3xl w-full max-h-[90vh] p-6 sm:p-8 flex flex-col justify-between z-[101] animate-in zoom-in-95 duration-200 relative overflow-hidden my-auto">
        
        {/* Header */}
        <div className="flex items-start justify-between border-b border-gray-100 pb-5 shrink-0">
          <div className="flex items-center gap-3.5">
            ...
            <div>
              <h3 className="text-lg font-extrabold text-gray-900 leading-tight tracking-tight">
                Import Data Pegawai
              </h3>
          ...
          <button
            ...
            disabled={uploading}
            onClick={onClose}
            className="p-2 text-gray-400 hover:text-gray-700 rounded-xl hover:bg-zinc-100 transition-colors cursor-pointer disabled:opacity-40 shrink-0"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto py-5 space-y-6 text-left pr-1">
            ...
            <div className="space-y-4">
              {!selectedFile ? (
                <div
                  ...
                  className={`border-2 border-dashed rounded-3xl p-10 sm:p-12 text-center cursor-pointer transition-all duration-200 flex flex-col items-center justify-center space-y-4 ${
                    isDragging
                      ? "border-[#5C8A90] bg-[#5C8A90]/10 scale-[1.01] shadow-lg shadow-[#5C8A90]/10"
                      : "border-gray-300 hover:border-[#5C8A90] bg-zinc-50/80 hover:bg-[#5C8A90]/5"
                  }`}
                >
                  <div
                    className="w-16 h-16 rounded-2xl flex items-center justify-center transition-transform duration-200 shadow-2xs"
                    style={{
                ...
                <div className="bg-emerald-50/80 border border-emerald-200 rounded-2xl p-5 flex items-center justify-between animate-in fade-in duration-200 shadow-2xs">
        ...
        {uploadResult && (() => {
          ...
          const errorsList: any[] = Array.isArray(review.errors)
            ...
            ? review.issues
            : [];
          const errorCount = review.error_count ?? review.failed_count ?? errorsList.length ?? 0;
          const totalRows = (review.total_rows ?? review.total) !== undefined
            ? (review.total_rows ?? review.total)
            : (employeesCreated + errorCount);
          const hasErrors = errorCount > 0 || errorsList.length > 0;

          return (
            <div className="space-y-4 animate-in fade-in duration-300">
              {/* Compact Top Status Alert */}
              {hasErrors ? (
                <div className="bg-rose-50/90 border border-rose-200/90 rounded-2xl px-4 py-3 flex items-center justify-between gap-3 text-left">
                  <div className="flex items-center gap-2.5 min-w-0">
                    <AlertCircle className="w-4 h-4 text-rose-600 shrink-0" />
                    <h4 className="text-xs font-bold text-rose-900 truncate">
                      Import Ditolak — Ditemukan {errorCount} Kesalahan Data
                    </h4>
                  </div>
                  <span className="px-2.5 py-0.5 rounded-md bg-rose-100/90 text-rose-800 font-bold text-[10px] font-mono shrink-0">
                    Batch #{batchId}
                  </span>
                </div>
              ) : (
                <div className="bg-emerald-50/90 border border-emerald-200/90 rounded-2xl px-4 py-3 flex items-center justify-between gap-3 text-left">
                  <div className="flex items-center gap-2.5 min-w-0">
                  ...
                  <span className="px-2.5 py-0.5 rounded-md bg-emerald-100/90 text-emerald-800 font-bold text-[10px] font-mono shrink-0">
                    Batch #{batchId}
                  </span>
                </div>
              )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
                ...
                <div className="md:col-span-5 space-y-3 flex flex-col justify-between">
                  {/* Ringkasan Metrics */}
                  <div className="bg-zinc-50/80 border border-zinc-200/80 rounded-2xl p-3.5 space-y-2.5 text-left">
                    <h5 className="text-[10.5px] font-extrabold text-gray-600 uppercase tracking-wider">
                      Ringkasan Hasil Import
                    </h5>
                    <div className="grid grid-cols-3 gap-2 text-center">
                      <div className="p-2 bg-white border border-gray-200/80 rounded-xl space-y-0.5">
                        <p className="text-[9px] text-gray-400 font-bold uppercase tracking-wider">Total</p>
                        <p className="text-base font-extrabold text-gray-800">
                          {totalRows}
                        </p>
                      </div>
                      <div className="p-2 bg-white border border-emerald-200/80 rounded-xl space-y-0.5">
                        <p className="text-[9px] text-emerald-600 font-bold uppercase tracking-wider">Berhasil</p>
                        <p className="text-base font-extrabold text-emerald-700">
                          {employeesCreated}
                        </p>
                      </div>
                      <div className="p-2 bg-white border border-rose-200/80 rounded-xl space-y-0.5">
                  ...
                  <div className="bg-zinc-50/80 border border-zinc-200/80 rounded-2xl p-3.5 space-y-2.5 text-left flex-1 flex flex-col justify-between">
                    ...
                    <Button
                      ...
                      className="w-full h-8.5 px-3 text-xs font-bold rounded-xl text-white shadow-xs hover:opacity-90 transition-all shrink-0 flex items-center justify-center gap-1.5 border-0 cursor-pointer"
                    >
                ...
                <div className="md:col-span-7">
                  {errorsList.length > 0 ? (
                    <div className="bg-rose-50/30 border border-rose-200/70 rounded-2xl p-3.5 space-y-2.5 text-left h-full flex flex-col">
                      <div className="flex items-center justify-between border-b border-rose-200/60 pb-2 shrink-0">
                      ...
                      <div className="space-y-2 max-h-64 overflow-y-auto pr-1 flex-1">
                        {errorsList.map((errItem: any, idx: number) => {
                          ...
                          return (
                            <div
                              key={idx}
                              className="bg-white border border-rose-200/70 rounded-xl p-2.5 text-xs space-y-1 shadow-2xs hover:border-rose-300 transition-all"
                            >
                    ...
                    <div className="bg-emerald-50/40 border border-emerald-200/70 rounded-2xl p-6 text-center h-full flex flex-col items-center justify-center space-y-2">
        ...
        <div className="flex items-center justify-end gap-3 pt-4 border-t border-gray-100 shrink-0">
          <Button
            ...
            disabled={uploading}
            onClick={onClose}
            className="h-10 px-5 text-xs font-bold rounded-xl text-gray-700 bg-white border border-gray-200 hover:bg-gray-50 cursor-pointer"
          >
          ...
          {!uploadResult && (
            <Button
              ...
              className={`h-10 px-6 text-xs font-bold rounded-xl text-white transition-all shadow-sm flex items-center justify-center gap-2 border-0 ${
          ...
          {uploadResult && (
            <Button
              ...
              className="h-10 px-5 text-xs font-bold rounded-xl bg-zinc-100 hover:bg-zinc-200 text-gray-700 border-0 cursor-pointer flex items-center gap-2"
            >
