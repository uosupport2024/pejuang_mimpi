...
export function EmployeeEditPage() {
  ...
  useEffect(() => {
    ...
    fetchEmployeeById(employeeId)
      .then((emp) => {
        setFormData({
          name: emp.name || "",
          foto_karyawan: emp.foto_karyawan || "",
          email: emp.email || "",
          telepon: emp.telepon || "",
          username: emp.username || "",
          password: "",
          tenant_id: emp.tenant_id ? String(emp.tenant_id) : "1",
          lokasi_id: emp.lokasi_id ? String(emp.lokasi_id) : "",
          jabatan_id: emp.jabatan_id ? String(emp.jabatan_id) : "",
          tgl_lahir: emp.tgl_lahir || "",
          tgl_join: emp.tgl_join || "",
          masa_berlaku: emp.masa_berlaku || "",
          role_name: emp.roles?.[0]?.name || emp.is_admin || "staff",
          gender: emp.gender || "Laki-Laki",
          is_admin: emp.is_admin || "user",

          // Status & Jabatan
          status_nikah: emp.status_nikah || "TK/0",
          status_pajak: emp.status_pajak || "TK/0",

          // Dokumen Identitas
          ktp: emp.ktp || "",
          kartu_keluarga: emp.kartu_keluarga || "",
          ...
          pkwt: emp.pkwt || "",
          kontrak: emp.kontrak || "",
          tgl_mulai_pkwt: emp.tgl_mulai_pkwt || "",
          tgl_berakhir_pkwt: emp.tgl_berakhir_pkwt || "",
          rekening: emp.rekening || "",
          bank: emp.bank || "",
          nama_rekening: emp.nama_rekening || "",

          // Cuti & Izin
          izin_cuti: emp.izin_cuti !== undefined && emp.izin_cuti !== null ? String(emp.izin_cuti) : "12",
          izin_lainnya: emp.izin_lainnya !== undefined && emp.izin_lainnya !== null ? String(emp.izin_lainnya) : "0",
          izin_telat: emp.izin_telat !== undefined && emp.izin_telat !== null ? String(emp.izin_telat) : "0",
          izin_pulang_cepat: emp.izin_pulang_cepat !== undefined && emp.izin_pulang_cepat !== null ? String(emp.izin_pulang_cepat) : "0",

          // Penjumlahan Gaji
          gaji_harian: emp.gaji_harian !== undefined && emp.gaji_harian !== null ? String(emp.gaji_harian) : "0",
          gaji_pokok: emp.gaji_pokok !== undefined && emp.gaji_pokok !== null ? String(emp.gaji_pokok) : "0",
          makan_transport: emp.makan_transport !== undefined && emp.makan_transport !== null ? String(emp.makan_transport) : "0",
          lembur: emp.lembur !== undefined && emp.lembur !== null ? String(emp.lembur) : "0",
          kehadiran_full: emp.kehadiran_full !== undefined && emp.kehadiran_full !== null ? String(emp.kehadiran_full) : "0",
          thr: emp.thr !== undefined && emp.thr !== null ? String(emp.thr) : "0",
          bonus_pribadi: emp.bonus_pribadi !== undefined && emp.bonus_pribadi !== null ? String(emp.bonus_pribadi) : "0",
          bonus_team: emp.bonus_team !== undefined && emp.bonus_team !== null ? String(emp.bonus_team) : "0",
          bonus_jackpot: emp.bonus_jackpot !== undefined && emp.bonus_jackpot !== null ? String(emp.bonus_jackpot) : "0",

          // Pengurangan Gaji
          potong_izin: emp.potong_izin !== undefined && emp.potong_izin !== null ? String(emp.potong_izin) : "0",
          potong_terlambat: emp.potong_terlambat !== undefined && emp.potong_terlambat !== null ? String(emp.potong_terlambat) : "0",
          potong_mangkir: emp.potong_mangkir !== undefined && emp.potong_mangkir !== null ? String(emp.potong_mangkir) : "0",
          saldo_kasbon: emp.saldo_kasbon !== undefined && emp.saldo_kasbon !== null ? String(emp.saldo_kasbon) : "0",

          // Tunjangan & Potongan Pajak / BPJS
          tunjangan_bpjs_kesehatan: emp.tunjangan_bpjs_kesehatan !== undefined && emp.tunjangan_bpjs_kesehatan !== null ? String(emp.tunjangan_bpjs_kesehatan) : "0",
          tunjangan_bpjs_ketenagakerjaan: emp.tunjangan_bpjs_ketenagakerjaan !== undefined && emp.tunjangan_bpjs_ketenagakerjaan !== null ? String(emp.tunjangan_bpjs_ketenagakerjaan) : "0",
          potongan_bpjs_kesehatan: emp.potongan_bpjs_kesehatan !== undefined && emp.potongan_bpjs_kesehatan !== null ? String(emp.potongan_bpjs_kesehatan) : "0",
          potongan_bpjs_ketenagakerjaan: emp.potongan_bpjs_ketenagakerjaan !== undefined && emp.potongan_bpjs_ketenagakerjaan !== null ? String(emp.potongan_bpjs_ketenagakerjaan) : "0",
          tunjangan_pajak: emp.tunjangan_pajak !== undefined && emp.tunjangan_pajak !== null ? String(emp.tunjangan_pajak) : "0",
        });
        setLoadingEmployee(false);
      })
      .catch((err) => {
        console.error(err);
        toast.error("Gagal memuat data detail pegawai.");
        setLoadingEmployee(false);
      });
  }, [employeeId]);
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      toast.success(`Pegawai "${formData.name}" berhasil dihapus.`);
  ...
  const isLoading = loadingMasters || loadingEmployee;
    ...
    <div className="w-full space-y-6">
      ...
      {isLoading ? (
        <div className="space-y-6">
          {/* Skeleton Left: Profile Card */}
          <div className="bg-white border border-gray-200/80 rounded-2xl p-6 space-y-4 shadow-xs animate-pulse">
            {/* Top Row Skeleton */}
            <div className="flex flex-col md:flex-row items-center md:items-start gap-4">
              ...
              <div className="flex flex-row gap-2.5 w-full md:w-auto shrink-0 md:pt-1">
            ...
            {/* Bottom Row Skeleton */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 border border-gray-100 rounded-xl p-3 w-full">
          ...
          <div className="h-11 w-full bg-white border border-gray-200/80 rounded-xl animate-pulse" />
        ...
        <div className="space-y-6">
          {/* Horizontal Profile Summary Card */}
          <div className="bg-white border border-gray-200/80 rounded-2xl shadow-xs p-6 flex flex-col md:flex-row items-start gap-6">
            {/* Profile Image / Silhouette Placeholder */}
            <div className="w-28 h-28 bg-gray-200 rounded-xl overflow-hidden flex items-center justify-center relative border border-gray-300 shrink-0">
              <svg className="w-full h-full text-white" fill="currentColor" viewBox="0 0 24 24">
                <path d="M24 20.993V24H0v-2.996A14.977 14.977 0 0112.004 15c4.904 0 9.26 2.354 11.996 5.993zM16.002 8.999a4 4 0 11-8 0 4 4 0 018 0z" />
              </svg>
            </div>

            {/* Right: Info Content & Actions */}
            <div className="flex-1 w-full space-y-3">
              {/* Top Row: Name/Title & Action Buttons */}
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                {/* Name and Job Title */}
                <div className="text-center md:text-left">
                  <h2 className="text-xl font-bold text-gray-800 leading-tight">{formData.name || "-"}</h2>
                  <p className="text-xs text-gray-400 font-medium mt-1">
                    {masters?.jabatan.find(j => String(j.id) === formData.jabatan_id)?.nama_jabatan || "Staff"}
                  </p>
                </div>

                {/* Actions Section */}
                <div className="flex flex-row gap-2.5 w-full md:w-auto justify-center md:justify-end shrink-0">
                  <button
                    type="button"
                    onClick={() => toast.info(`Input Shift untuk ${formData.name}`)}
                    className="flex-1 md:flex-initial flex items-center justify-center gap-2 px-4 py-2 bg-emerald-50 text-emerald-600 hover:bg-emerald-100 rounded-lg text-xs font-bold transition-colors cursor-pointer"
                  >
                  ...
                  <button
                    ...
                    className="flex-1 md:flex-initial flex items-center justify-center gap-2 px-4 py-2 bg-red-50 text-red-600 hover:bg-red-100 rounded-lg text-xs font-bold transition-colors cursor-pointer"
                  >
                    <Trash2 size={14} />
                    Delete
                  </button>
              ...
              {/* Bottom Row: Info Grid (Under Name, not under Avatar) */}
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 border border-gray-100 rounded-xl p-3 text-xs bg-zinc-50/50 w-full">
                <div className="flex flex-col gap-0.5">
                  <span className="font-bold text-gray-800">Email</span>
                  <span className="text-gray-500 font-medium truncate" title={formData.email}>{formData.email || "-"}</span>
                </div>
                <div className="flex flex-col gap-0.5 border-t md:border-t-0 md:border-l border-gray-100 pt-2 md:pt-0 md:pl-4">
                  <span className="font-bold text-gray-800">Username</span>
                  <span className="text-gray-500 font-medium">{formData.username || "-"}</span>
                </div>
                <div className="flex flex-col gap-0.5 border-t md:border-t-0 md:border-l border-gray-100 pt-2 md:pt-0 md:pl-4">
                  <span className="font-bold text-gray-800">Telepon</span>
                  <span className="text-gray-500 font-medium">{formData.telepon || "-"}</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <ConfirmationModal
        ...
        title="Hapus Pegawai"
        message={`Apakah Anda yakin ingin menghapus pegawai "${formData.name}"? Semua data yang terkait dengan pegawai ini akan terhapus secara permanen.`}
        variant="danger"
        loading={deleting}
      />
    </div>
