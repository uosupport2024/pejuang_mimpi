...
export function SlipGajiBreakdown({ user, formatRupiah }: SlipGajiBreakdownProps) {
  ...
  return (
    <div className="bg-white p-4.5 rounded-2xl border border-gray-100 shadow-xs space-y-3.5 text-left">
      <h3 className="text-xs font-extrabold text-gray-950 uppercase tracking-wide px-0.5">Rincian Slip Gaji</h3>
      ...
      <div className="divide-y divide-gray-100 text-xs font-semibold space-y-2.5">
        <div className="flex justify-between items-center pt-2">
          ...
          <span className="text-gray-900 font-extrabold">{formatRupiah(gajiPokok)}</span>
        </div>
        <div className="flex justify-between items-center pt-2">
          <span className="text-zinc-500 font-bold">Uang Lembur</span>
          <span className="text-emerald-600 font-extrabold">+{formatRupiah(lembur)}</span>
        </div>
        <div className="flex justify-between items-center pt-2">
          <span className="text-zinc-500 font-bold">Potongan Izin</span>
          <span className="text-rose-500 font-extrabold">-{formatRupiah(izin)}</span>
        </div>
        <div className="flex justify-between items-center pt-2">
          <span className="text-zinc-500 font-bold">Potongan Terlambat</span>
          <span className="text-rose-500 font-extrabold">-Rp 0</span>
        </div>
        <div className="flex justify-between items-center pt-2.5 border-t border-gray-200 font-black text-gray-950">
